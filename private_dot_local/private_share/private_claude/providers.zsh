...
ccoa() {
  ...
  local ANTHROPIC_DEFAULT_FABLE_MODEL_DESCRIPTION="Latest Fable through Anthropic 1P"
  local ANTHROPIC_DEFAULT_OPUS_MODEL_DESCRIPTION="Latest Opus through Anthropic 1P"
  local ANTHROPIC_DEFAULT_SONNET_MODEL_DESCRIPTION="Latest Sonnet through Anthropic 1P"
  local ANTHROPIC_DEFAULT_HAIKU_MODEL_DESCRIPTION="Latest Haiku through Anthropic 1P"
...
cct() {
  if [[ -z "${TOKENATOR_API_KEY:-}" ]]; then
    print -u2 "TOKENATOR_API_KEY is not set"
    return 1
  fi

  local ANTHROPIC_BASE_URL="https://api.tokenator.cloud/anthropic"
  local ANTHROPIC_AUTH_TOKEN="$TOKENATOR_API_KEY"
  local ANTHROPIC_DEFAULT_FABLE_MODEL="claude-fable-5[1m]"
  local ANTHROPIC_DEFAULT_OPUS_MODEL="claude-opus-4-8[1m]"
  local ANTHROPIC_DEFAULT_SONNET_MODEL="claude-sonnet-4-6[1m]"
  local ANTHROPIC_DEFAULT_HAIKU_MODEL="claude-haiku-4-5"
  local ANTHROPIC_MODEL="sonnet"
  local CLAUDE_CODE_SUBAGENT_MODEL="$ANTHROPIC_MODEL"
  local CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY="1"
  local CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC="1"
  local -a claude_env=(
    "ANTHROPIC_BASE_URL=$ANTHROPIC_BASE_URL"
